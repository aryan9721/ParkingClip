...
const VehicleRecord = () => {
  ...
  function exitvehicle(){
    ...
    axios.put(exitURL+ticketData.ticketId+'/EXITED').then((response) => {
        // console.log('exit',response.data.message);
        setShowModal(false);
    setExitresult(response.data.message);

    //   results.entryDateTime = new Date(results.entryDateTime);
    });
}
function forceExit(row) {
  const exitURL = api+ "api/parkings/vehicle/ticketStatus/";
  axios.put(exitURL+row.ticketId+'/FORCE_EXIT').then((response) => {
      console.log('FORCE ',response);
  setExitresult(response.data.message);
  //   results.entryDateTime = new Date(results.entryDateTime);
  });
}
  ...
  const data = {
    ...
    rows: results && results.map((row) => ({ // Add a check before calling map
      ...
      forceExit: row.parkingStatus=="PARKED" ? <button onClick={() => forceExit(row)}>Force Exit</button> : "NA"
    })),
  };
  // console.log(data);
        ...
        ticketData ? (<Modal open={showModal} onClose={() => setShowModal(false)}>
          {/* <div className='container'> */}
            ...
            <span className='headingText'>Parking Clip</span>
              ...
              {ticketData.parkingStatus == "PARKED" ? (<div style={{ display: 'flex', justifyContent: 'center', marginBottom: 10 }}>
